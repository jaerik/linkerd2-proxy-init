...
func addOutgoingTrafficRules(commands []*exec.Cmd, firewallConfiguration FirewallConfiguration) []*exec.Cmd {
	...
	if err != nil {
		log.Printf("An error occurred while FLUSHING the chain. Startup will continue, but there may be additional errors\n [error]: %v", err)
	}

	err = executeCommand(firewallConfiguration, makeDeleteChain(outputChainName))
	if err != nil {
		log.Printf("An error occurred while DELETING the chain. Startup will continue, but there may be additional errors\n [error]: %v", err)
	}
